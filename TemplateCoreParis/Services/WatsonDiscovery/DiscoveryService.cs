...
namespace IBM.WatsonDeveloperCloud.Discovery.v1
{
    public class DiscoveryService : WatsonService, IDiscoveryService
    {
        ...
        public Configuration2 CreateConfiguration(string environmentId, Configuration2 configuration)
        {
            if (string.IsNullOrEmpty(environmentId))
                throw new ArgumentNullException(nameof(environmentId));
            ...
            try
            {
                result = this.Client.WithAuthentication(this.UserName, this.Password)
                                ...
                                .WithArgument("version", VersionDate)
                                .WithBody<Configuration2>(configuration)
                                .As<Configuration2>()
                                .Result;
            }
            catch(AggregateException ae)
        ...
        public Configuration2 GetConfiguration(string environmentId, string configurationId)
        {
            if (string.IsNullOrEmpty(environmentId))
                throw new ArgumentNullException(nameof(environmentId));
            if (string.IsNullOrEmpty(configurationId))
        ...
        public Configuration2 UpdateConfiguration(string environmentId, string configurationId, Configuration2 configuration)
        {
            if (string.IsNullOrEmpty(environmentId))
                throw new ArgumentNullException(nameof(environmentId));
            if (string.IsNullOrEmpty(configurationId))
