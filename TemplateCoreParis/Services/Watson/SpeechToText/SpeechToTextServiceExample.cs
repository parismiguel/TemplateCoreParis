...
using IBM.VCA.Watson.Watson.SpeechToText.Model;
using System;
...
namespace IBM.VCA.Watson.Watson.SpeechToText
{
    public class SpeechToTextServiceExample
    {
        ...
        static string _modelToGet = "es-ES_BroadbandModel";
        private string _sessionID;

        static string _username = "e195aa9f-57e4-4328-a815-f08311ad9b3f";
        static string _password = "LVSk4sEVs2Iz";

        public SpeechToTextServiceExample(string username, string password)
        {
            _speechToText.SetCredential(_username, _password);
        ...
        public static List<string> Recognize2(string path)
        {
            ...
            using (FileStream fs = File.OpenRead(path))
            {
                ...
                Console.WriteLine("Calling Recognize...");
                //_speechToText.CreateSession(_modelToGet);
                var speechEvent = _speechToText.Recognize(fs);
                ...
                if (speechEvent.Results != null || speechEvent.Results.Count > 0)
                {
                    foreach (SpeechRecognitionResult result in speechEvent.Results)
                    {
                        if (result.Alternatives != null && result.Alternatives.Count > 0)
                        {
                            foreach (SpeechRecognitionAlternative alternative in result.Alternatives)
                            {
                                list.Add(alternative.Transcript);
                                Console.WriteLine(string.Format("{0}, {1}", alternative.Transcript, alternative.Confidence));
                            }
                        }
                    }
